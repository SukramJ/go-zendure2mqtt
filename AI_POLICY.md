...
go-zendure2mqtt. It is modeled on the
...
gates: `go vet ./...`, `gofumpt` formatting, `golangci-lint run ./...`, and the race-enabled
test suite (`go test -race -count=1 ./...`) — bundled locally as `make check` and enforced by
CI on every pull request. AI assistance does not lower the bar.

## How this project itself uses AI

In the interest of transparency: go-zendure2mqtt is partly developed with AI assistance
